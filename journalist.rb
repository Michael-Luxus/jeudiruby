...
puts "Il y a #{twitter.length} journalistes dans cet array."



contain_number = 0
twitter.each do |entree| 
    if entree =~ /\d/
        contain_number += 1
    end
end
puts "Il y a #{contain_number} handles qui contiennent de numéro"
...
total = min + maj
puts "Il y a #{total} entrée(s) incluant le mot \"Aude\" ou \"aude\""



majuscule = 0
twitter.each do |entree| 
    if entree.slice(1) =~ /[A-Z]/
        majuscule += 1
    end
end
puts "Il y a #{majuscule} qui commencent par un majuscule"
...
puts "Il y a #{contientCapital} qui contient une majuscules"



underscore = 0
twitter.each do |entree| 
    underscore += entree.scan(/_/).count
end
puts "Il y a #{underscore} \"_\" dans tous les pseudos confondus"



puts "Voici la liste alphabétique :"
puts twitter.sort
